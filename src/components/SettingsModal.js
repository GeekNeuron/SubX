...
function SettingsModal({ isOpen, onClose, setNotification }) {
    ...
    const handleErrorChange = (e) => {
        ...
        setLocalErrorConfig(prev => ({ ...prev, [name]: parseInt(value, 10) }));
    };
    ...
    const handleSave = () => {
        updateErrorConfig(localErrorConfig);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100] p-4">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-6">
                    <section>
                        <h3 className="text-lg font-medium mb-3 text-sky-600 dark:text-sky-400">General Settings</h3>
                        <div className="space-y-2">
                             <label className={checkboxLabelClass}>
                                <input type="checkbox" name="autosave" checked={localAppearanceConfig.autosave} onChange={handleAppearanceChange} className={checkboxClass} />
                                Enable Autosave
                            </label>
                        </div>
                    </section>
                    <section>
                        <h3 className="text-lg font-medium mb-3 text-sky-600 dark:text-sky-400">Error Checking Settings</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div><label htmlFor="MIN_DURATION_MS" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Min. Duration (ms)</label><input type="number" name="MIN_DURATION_MS" id="MIN_DURATION_MS" value={localErrorConfig.MIN_DURATION_MS} onChange={handleErrorChange} className={inputClass} /></div>
                            <div><label htmlFor="MAX_DURATION_MS" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Max. Duration (ms)</label><input type="number" name="MAX_DURATION_MS" id="MAX_DURATION_MS" value={localErrorConfig.MAX_DURATION_MS} onChange={handleErrorChange} className={inputClass} /></div>
                            <div><label htmlFor="MAX_LINES" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Max. Lines per Subtitle</label><input type="number" name="MAX_LINES" id="MAX_LINES" value={localErrorConfig.MAX_LINES} onChange={handleErrorChange} className={inputClass} /></div>
                            <div><label htmlFor="MAX_CHARS_PER_LINE" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Max. Chars per Line</label><input type="number" name="MAX_CHARS_PER_LINE" id="MAX_CHARS_PER_LINE" value={localErrorConfig.MAX_CHARS_PER_LINE} onChange={handleErrorChange} className={inputClass} /></div>
                            <div className="sm:col-span-2"><label htmlFor="OVERLAP_FIX_GAP_MS" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Overlap Fix Gap (ms)</label><input type="number" name="OVERLAP_FIX_GAP_MS" id="OVERLAP_FIX_GAP_MS" value={localErrorConfig.OVERLAP_FIX_GAP_MS} onChange={handleErrorChange} className={inputClass} /></div>
                        </div>
                    </section>
                    <section>
                        <h3 className="text-lg font-medium mb-3 text-sky-600 dark:text-sky-400">Appearance Settings</h3>
                        <div className="space-y-4">
                            <div>
                                <label htmlFor="tableFont" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Subtitle Table Font</label>
                                <select name="tableFont" id="tableFont" value={localAppearanceConfig.tableFont} onChange={handleAppearanceChange} className={selectClass}>
                                    <option value="font-sans">System Default</option>
                                    <option value="font-vazir">Vazirmatn (Persian)</option>
